...
class Sampler(object):
    ...
    def sample(self):
        ...
        for input_path in tqdm(input_paths):
            ...
            model.eval()
            clip_size_r = 2048 #512#1024 512+256
                ...
                for i in tqdm(range(rows_iters)):
                    for j in range(cols_iters):
                        ...
                        if np.all(data == 0):
                            ...
                            vflip = T.RandomVerticalFlip(p=1)

                            # 原始预测
                            original_prediction = self.slide_sample(cond,
                                                           crop_size=self.cfg.sampler.get('crop_size', [320, 320]),
                                                           stride=self.cfg.sampler.stride, mask=mask2)

                            # seg_original = original_prediction.detach().cpu().numpy()
                            # print(seg_original)

                            # 水平翻转预测
                            flipped_cond_h = hflip(cond)
                            flipped_prediction_h = self.slide_sample(flipped_cond_h,
                                                           crop_size=self.cfg.sampler.get('crop_size', [320, 320]),
                                                           stride=self.cfg.sampler.stride, mask=mask2)
                            # 将水平翻转的预测结果翻转回来
                            flipped_prediction_h = hflip(flipped_prediction_h)

                            # seg_hflip = flipped_prediction_h.detach().cpu().numpy()

                            # 垂直翻转预测
                            flipped_cond_v = vflip(cond)
                            flipped_prediction_v = self.slide_sample(flipped_cond_v,
                                                           crop_size=self.cfg.sampler.get('crop_size', [320, 320]),
                                                           stride=self.cfg.sampler.stride, mask=mask2)
                            # 将垂直翻转的预测结果翻转回来
                            flipped_prediction_v = vflip(flipped_prediction_v)

                            # seg_vflip =  flipped_prediction_v.detach().cpu().numpy()

                            # 结合原始预测和翻转预测结果
                            # seg = (seg_original + seg_hflip + seg_vflip) / 3
                            combined_prediction = torch.max(torch.max(original_prediction, flipped_prediction_h),
                                                            flipped_prediction_v)
                            seg = combined_prediction.detach().cpu().numpy()
